...
const AddSong = () => {
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${PROXY_URL}/api/song/add`, {
        method : 'POST', 
        credentials : 'include', 
        body : formData
      });
  ...
  return loading ? (
    ...
    <form onSubmit={onSubmitHandler} className='flex flex-col items-start gap-8 text-gray-600'>
      ...
      <div className="flex flex-col gap-2.5">
        <p>Song name</p>
        <input className='bg-transparent outline-green-600 border-2 mr-2 border-gray-400 p-2.5 w-[max(40vw,250px)]' onChange={(e) => setName(e.target.value)} value={name} type="text" placeholder='Type here' required />
      </div>

      <div className="flex flex-col gap-2.5">
        <p>Song description</p>
        <input className='bg-transparent outline-green-600 border-2 mr-2 border-gray-400 p-2.5 w-[max(40vw,250px)]' onChange={(e) => setDesc(e.target.value)} value={desc} type="text" placeholder='Type here' required />
      </div>
